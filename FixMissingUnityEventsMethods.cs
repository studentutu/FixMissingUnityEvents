...
{
	private static Dictionary<string, List<string>> scriptNameToListOfNameSpaces = new Dictionary<string, List<string>>();
	...
	private const string CACHED_ARGUMENTS = "m_Arguments";
	private const string SHORT_TYPE_ASSEMBLY = "m_ObjectArgumentAssemblyTypeName";
	...
	[FormerlySerializedAs("changedDOne")]
	[SerializeField] 
	...
	private void FixMethods()
	{
		...
		foreach (var assemblyIn in assemblyCurrent)
		{
			...
			foreach (var targetType in types)
			{
				...
				// TransfrVR.Workflow.ProgramAce.Common.TimelineAPIToPublic, Workflow.ProgramAce
		...
		foreach (var refactored in changedInto)
		{
			...
			else
			{
				...
				scriptNameToListOfNameSpaces[refactored.OriginalClass].AddRange(scriptNameToListOfNameSpaces[refactored.RenamedIntoClass]);
		...
		for (int i = 0; i < allRootObjects.Length; i++)
		{
			...
			EditorUtility.DisplayProgressBar("Refactoring Progress",allRootObjects[i].name, i/allRootObjects.Length );
	...
	private static void GoThroughtGameObjects(GameObject mainGO, float currentProgrss)
	{
		...
		for (int i = 0; i < mainGO.transform.childCount; i++)
		{
			if (currentProgrss >= 0)
			{
				EditorUtility.DisplayProgressBar("Refactoring Progress",mainGO.transform.GetChild(i).name,  (i*diff)/currentProgrss );
			}
	...
	private static void GoThroughtSerializedObjects(GameObject go)
	{
	...
	private static void GoInRecursion(SerializedObject so)
	{
	...
	private static void ObserveSerializedProperty(SerializedProperty property)
	{
		// DO seek 
		CHeckTheUnityEventFunctions(property);
	}

	private static void CHeckTheUnityEventFunctions(SerializedProperty objectToLookAsUnityEvent )
	{
					...
					if (call != null)
					{
						...
						if (argumentAssemblyType == typeOfUnityObject)
						{
							...
							if (scriptNameToListOfNameSpaces.ContainsKey(prevType))
							{
								...
								Debug.LogWarning("Changed " + objectToLookAsUnityEvent.serializedObject.targetObject.name, objectToLookAsUnityEvent.serializedObject.targetObject);
							}
						}
					}
				}
